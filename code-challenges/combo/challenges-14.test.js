...
const toTitleCase = (arr) => {
  let results = [];
  arr.forEach( string => {
    let upperString = string.replace(/^\w/, letter => letter.toUpperCase())
    results.push(upperString);
  });
  return results;
};
...
let biggerThanLuke = (arr) => {
  ...
  let biggest = arr[0];
  arr.forEach( object => {
    if (parseInt(biggest.mass) < parseInt(object.mass)){
      bigs.push(object.name)
    }
  })
  return bigs.join(' - ');
};
