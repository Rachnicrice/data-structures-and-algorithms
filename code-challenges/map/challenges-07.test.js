...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let raisedNum = Math.pow(2, arr[i]);
    raisedArr.push(raisedNum);
  }
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach( number => {
    let raisedNum = Math.pow(2, number);
    raisedArr.push(raisedNum);
  })
  return raisedArr;
};
...
const mapTwoToThe = (arr) => {
  let raisedArr = arr.map( number => Math.pow(2, number));
  return raisedArr;
};
...
const extractAbilities = (arr) => {
  let abilityArr = arr.map(pokemon => pokemon.ability.name)
  return abilityArr;
};
...
const extractStats = (arr) => {
  let statArr = arr.map(function (stats){
    let thing = new Object ();
    thing.name = stats.stat.name
    thing.total = stats.effort + stats.baseStat
