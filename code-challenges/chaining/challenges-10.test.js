...
const count = (target, input) => {
  return input.reduce((accumulator, array) => {
    let num = array.reduce( (accumulator, value) => {
      if (value === target) {
        accumulator++;
      }
      return accumulator;
...
const totalSum = (input) => {
  return input.reduce( (total, array) => {
    return total + array.reduce( (accumulator, value) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map( array => {
    return array.filter( value => typeof value === 'number' && value % 5 === 0).map( number => {
...
let findMaleAndFemale = (data) => {
  return data.filter(object => object.gender === 'male' || object.gender === 'female').reduce((accumulator, character) => {
...
let findShortest = (data) => {
  ...
  return data.reduce((accumulator, value) => {
    if (parseInt(value.height, 10) < parseInt(shortest)) {
      shortest = value.height;
